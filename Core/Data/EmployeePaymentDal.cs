...
namespace Core.Data
{
    internal class EmployeePaymentDal : Dal
    {
        ...
        internal void InsertEmployeePayment(EmployeePayment employeePayment, uint userId)
        {
            // Define sql command
            var command = new CommandDefinition(
                "insert into employees_payments (id_employee, amount, note, id_user) values (@id_employee, @amount, @note, @id_user)",
                new
                {
        ...
        internal IEnumerable<EmployeePayment> GetEmployeePayments(uint employeeId, uint recordLimit)
        {
            // Define sql command
            var command = new CommandDefinition(
                "select id_payment 'Id', id_employee 'EmployeeId', amount 'Amount', note 'Note', date_payment 'Date', id_user 'UserId' " +
