    ...
    internal class ItemReturnDal : Dal
    {
        ...
        internal void InsertItemReturn(ItemReturn itemReturn, uint userId)
        {
            ...
            var command = new CommandDefinition(
                "insert into items_returns (id_item, id_customer, qty_return, condition_return, note, id_user) " +
                "values (@id_item, @id_customer, @qty_return, @condition_return, @note, @id_user)",
                new
                {
                    id_item = itemReturn.ItemId,
                    id_customer = itemReturn.CustomerId,
                    qty_return = itemReturn.ReturnQty,
                    condition_return = itemReturn.Condition,
        ...
        /// </summary>
        /// <param name="itemReturn"></param>
        internal void UpdateItemReturnDetails(ItemReturn itemReturn)
        {
            // Define sql command
            var command = new CommandDefinition(
                "update items_returns set id_item = @id_item, qty_return = @qty_return, note = @note " +
        ...
        /// <summary>
        ///     Updates an existing item return's condition in database
        /// </summary>
        /// <param name="itemReturnId"></param>
        /// <param name="newCondition"></param>
        internal void UpdateItemReturnCondition(uint itemReturnId, ReturnCondition newCondition)
        {
            // Define sql command
            var command = new CommandDefinition(
                "update items_returns set condition_return = @condition_return where id_return = @id_return",
                new
                {
                    id_return = itemReturnId,
                    condition_return = newCondition.ToString()
                });
        ...
        /// <param name="returnCondition">if null, return all irrespective of return condition, else return only given type</param>
        /// <param name="recordLimit">number of items returned</param>
        internal IEnumerable<ItemReturn> GetItemReturns(string note, ReturnCondition? returnCondition, uint recordLimit)
        {
            // Define sql command
            var command = new CommandDefinition(
                "select id_return 'Id', id_item 'ItemId', id_customer 'CustomerId', qty_return 'ReturnQty', " +
            ...
            var commandCondition = new CommandDefinition(
                "select id_return 'Id', id_item 'ItemId', id_customer 'CustomerId', qty_return 'ReturnQty', " +
                "date_return 'Date', condition_return-1 'ReturnCondition', note 'Note', id_user 'UserId' " +
                "from items_returns where note like @return_note and condition_return = @condition_return " +
                "order by id_return desc limit @limit",
                new
                {
                    return_note = note,
                    condition_return = returnCondition.ToString(),
                    limit = recordLimit
                });

            // Execute sql command
            return Connection.Query<ItemReturn>(returnCondition == null ? command : commandCondition);
        }
